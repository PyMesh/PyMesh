...
#include <Boolean/Carve/CarveEngine.h>

#include "../BooleanEngineTest.h"

class CarveEngineTest : public BooleanEngineTest {
    protected:
        BooleanPtr get_disjoint_setting(MeshPtr mesh) {
            ...
            carve_engine->set_mesh_1(vertices_1, faces_1);
            carve_engine->set_mesh_2(vertices_2, faces_1);
            return carve_engine;
        }
        ...
        BooleanPtr get_epsilon_box_setting(
            ...
            BooleanPtr carve_engine = BooleanEngine::create("carve");
            carve_engine->set_mesh_1(vertices, faces);
            carve_engine->set_mesh_2(box_vertices, box_faces);
            carve_engine->compute_intersection();
            return carve_engine;
        }
        ...
        void assert_interior(const MatrixFr& vertices, const MatrixIr& faces,
                const VectorF& p) {
            const Float EPS = 0.1;
            BooleanPtr carve_engine = get_epsilon_box_setting(vertices, faces, p, EPS);

            const MatrixFr& vts = carve_engine->get_vertices();
        ...
        void assert_exterior(const MatrixFr& vertices, const MatrixIr& faces,
            ...
            const Float EPS = 0.1;
            BooleanPtr carve_engine = get_epsilon_box_setting(vertices, faces, p, EPS);
            ASSERT_EQ(0, carve_engine->get_vertices().rows());
        }
...
TEST_F(CarveEngineTest, disjoint_union) {
    if (!BooleanEngine::supports("carve")) return;
    MeshPtr mesh = load_mesh("cube.obj");

    BooleanPtr carve_engine = get_disjoint_setting(mesh);
    carve_engine->compute_union();
...
TEST_F(CarveEngineTest, disjoint_intersection) {
    if (!BooleanEngine::supports("carve")) return;
    MeshPtr mesh = load_mesh("cube.obj");
...
TEST_F(CarveEngineTest, disjoint_difference) {
    if (!BooleanEngine::supports("carve")) return;
    MeshPtr mesh = load_mesh("cube.obj");

    BooleanPtr carve_engine = get_disjoint_setting(mesh);
    carve_engine->compute_difference();
...
TEST_F(CarveEngineTest, disjoint_symmetric_difference) {
    if (!BooleanEngine::supports("carve")) return;
    MeshPtr mesh = load_mesh("cube.obj");

    BooleanPtr carve_engine = get_disjoint_setting(mesh);
    carve_engine->compute_symmetric_difference();
...
TEST_F(CarveEngineTest, overlap_union) {
    if (!BooleanEngine::supports("carve")) return;
    MeshPtr mesh = load_mesh("cube.obj");

    BooleanPtr carve_engine = get_overlap_setting(mesh);
    carve_engine->compute_union();
...
TEST_F(CarveEngineTest, overlap_intersection) {
    if (!BooleanEngine::supports("carve")) return;
    MeshPtr mesh = load_mesh("cube.obj");
...
TEST_F(CarveEngineTest, overlap_difference) {
    if (!BooleanEngine::supports("carve")) return;
    MeshPtr mesh = load_mesh("cube.obj");
...
TEST_F(CarveEngineTest, overlap_symmetric_difference) {
    if (!BooleanEngine::supports("carve")) return;
    MeshPtr mesh = load_mesh("cube.obj");
...
TEST_F(CarveEngineTest, open_surface) {
    if (!BooleanEngine::supports("carve")) return;
    MeshPtr mesh = load_mesh("cube.obj");
    ...
    BooleanPtr carve_engine = BooleanEngine::create("carve");
    carve_engine->set_mesh_1(box_vertices, box_faces);
    carve_engine->set_mesh_2(tri_vertices, tri_faces);
    carve_engine->compute_intersection();
    ...
    // carve does not handle intersecting closed surface with open surface.
