...
void PLYWriter::write_mesh(Mesh& mesh) {
    ...
    assert_success(ply_write_header(ply));
...
void PLYWriter::add_face_elements_header(Mesh& mesh, p_ply& ply) {
    ...
    const size_t num_faces = mesh.get_num_faces();
    assert(num_faces > 0);
    assert_success(ply_add_element(ply, "face", num_faces));
    assert_success(ply_add_list_property(ply, "vertex_indices", PLY_UCHAR, PLY_INT));
...
void PLYWriter::add_voxel_elements_header(Mesh& mesh, p_ply& ply) {
    ...
    assert_success(ply_add_element(ply, "voxel", num_voxels));
    assert_success(ply_add_list_property(ply, "vertex_indices", PLY_UCHAR, PLY_INT));
