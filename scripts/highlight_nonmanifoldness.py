...
def main():
    ...
    is_nonmanifold = np.any(edge_manifold==0, axis=1);
    involved_faces = faces[is_nonmanifold];
    mesh.add_attribute("faces_adj_nonmanifold_edge");
    mesh.set_attribute("faces_adj_nonmanifold_edge", is_nonmanifold);

    print("Faces adjacent to nonmanifold edges");
    print(involved_faces);

    vertex_manifold = mesh.get_attribute("vertex_manifold").ravel();
    is_nonmanifold = np.any(vertex_manifold[faces] == 0, axis=1);
    involved_faces = faces[is_nonmanifold];
    mesh.add_attribute("faces_adj_nonmanifold_vertex");
    mesh.set_attribute("faces_adj_nonmanifold_vertex", is_nonmanifold);
